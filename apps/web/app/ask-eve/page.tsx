...
export default function AskEvePage() {
  ...
  const isSv = lang === "sv";
  const methodVersion = result?.methodology_version ?? "TS_V2_CMD_DDM_NP_V1";
    ...
    <div style={{ maxWidth: 900, margin: "0 auto", padding: "24px 16px" }}>
      ...
      <div className="card" style={{ marginTop: 16, padding: 20 }}>
        <div style={{ fontSize: 13, fontWeight: 700, color: "var(--text-primary)", marginBottom: 10 }}>
          {isSv ? "Vad denna rapport innehåller" : "What This Report Contains"}
        </div>

        <p style={{ fontSize: 11, color: "var(--text-secondary)", lineHeight: 1.6, margin: "0 0 10px" }}>
          {isSv
            ? "Denna rapport genereras deterministiskt från låsta EVE CMD-datasets. Systemet gör inga prognoser, simuleringar eller policyrekommendationer. Det beräknar reproducerbara värden från öppna regulatoriska datakällor:"
            : "This report is generated deterministically from locked EVE CMD datasets. It does not forecast, simulate, or provide policy recommendations. It computes reproducible values from publicly available regulatory data sources:"
          }
        </p>

        <div style={{ fontSize: 11, color: "var(--text-muted)", lineHeight: 1.8, marginBottom: 12, paddingLeft: 12 }}>
          • ENTSO-E Transparency Platform — {isSv ? "zonpriser, produktion, flöden" : "zone prices, generation, flows"}<br/>
          • Nord Pool Day-Ahead — {isSv ? "officiellt systempris" : "official system price"}<br/>
        ...
        <div style={{ display: "flex", gap: 8, flexWrap: "wrap", marginBottom: 16 }}>
          {[
            ...
            { en: "Linked to sealed datasets", sv: "Länkad till förseglade datasets", color: "#a855f7" },
