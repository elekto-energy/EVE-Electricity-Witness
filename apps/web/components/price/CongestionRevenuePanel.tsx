...
export default function CongestionRevenuePanel() {
  ...
  if (!data) return <div style={{ padding: 16, color: "var(--text-muted)" }}>Laddar flaskhalsdata…</div>;
  ...
  const srcBadge = (url: string, label: string) => (
    <a href={url} target="_blank" rel="noopener" style={{ fontSize: 10, color: "var(--text-muted)", textDecoration: "underline", textDecorationColor: "var(--border-color)" }}>
      {label}
    </a>
  );
  ...
  return (
    <section style={{ background: "var(--bg-card)", border: "1px solid var(--border-color)", borderRadius: 8, padding: 20, marginBottom: 24 }}>
      ...
      <div style={{ display: "flex", gap: 8, fontSize: 10, color: "var(--text-muted)", marginBottom: 16, flexWrap: "wrap" }}>
      ...
      <div style={{ marginBottom: 24 }}>
        {(() => {
          ...
          return <>
            <div className="congestion-bar-chart" style={{ display: "flex", alignItems: "flex-end", gap: 6, height: 280, paddingBottom: 4 }}>
              {allItems.map((item) => (
                <div key={item.year} className="congestion-bar-item" style={{ flex: 1, display: "flex", flexDirection: "column", alignItems: "center", height: "100%", justifyContent: "flex-end" }}>
                  ...
                  {/* Year */}
                  <div style={{ fontSize: 13, color: item.isCrisis ? "#ef4444" : item.isPrognos ? "#f59e0b" : "var(--text-primary)", marginTop: 4, fontWeight: 600 }}>
                    {item.year}{item.isPrognos ? "*" : ""}
                  </div>

                  {/* Source */}
                  <div style={{ fontSize: 8, color: "var(--text-muted)", marginTop: 1 }}>
                    <a href={item.sourceUrl} target="_blank" rel="noopener" style={{ color: "var(--text-muted)" }}>{item.sourceLabel} ✓</a>
                  </div>
                </div>
              ))}
            </div>
            ...
            {/* Legend */}
            <div style={{ display: "flex", gap: 16, fontSize: 11, color: "var(--text-muted)", marginTop: 10, flexWrap: "wrap" }}>
      ...
      <div style={{ background: "var(--bg-primary)", border: "1px solid var(--border-color)", borderRadius: 8, padding: 16, marginBottom: 16 }}>
        ...
        <div className="congestion-cards" style={{ display: "flex", gap: 12, marginBottom: 12 }}>
          <div style={{ flex: 1, background: "var(--bg-card)", border: "1px solid var(--border-color)", borderRadius: 8, padding: 12, textAlign: "center" }}>
            ...
            <div style={{ fontSize: 11, color: "var(--text-muted)" }}>mdr SEK</div>
            <div style={{ fontSize: 10, color: "var(--text-muted)" }}>per dec 2024</div>
            <div style={{ fontSize: 9, color: "var(--text-muted)", marginTop: 4 }}>
              <a href="https://ei.se/om-oss/nyheter/2025/2025-03-04-hoga-flaskhalsintakter-under-2024---sa-har-har-de-anvants" target="_blank" rel="noopener" style={{ color: "var(--text-muted)" }}>
                📄 Ei rapport 2024 ✓
              </a>
            </div>
          </div>
          <div style={{ flex: 1, background: "var(--bg-card)", borderRadius: 8, padding: 12, textAlign: "center", border: "1px solid #22c55e33" }}>
            <div style={{ fontSize: 11, color: "#22c55e" }}>Planerade investeringar</div>
            <div style={{ fontSize: 28, fontWeight: 700, color: "#22c55e" }}>57,0</div>
            <div style={{ fontSize: 11, color: "var(--text-muted)" }}>mdr SEK</div>
            <div style={{ fontSize: 10, color: "var(--text-muted)" }}>2026–2028 (3 år)</div>
            <div style={{ fontSize: 9, color: "var(--text-muted)", marginTop: 4 }}>
              📄 Svk investeringsplan ✓
            </div>
          </div>
          <div style={{ flex: 1, background: "var(--bg-card)", border: "1px solid var(--border-color)", borderRadius: 8, padding: 12, textAlign: "center" }}>
            <div style={{ fontSize: 11, color: "var(--text-secondary)" }}>Saldo efter plan</div>
            <div style={{ fontSize: 28, fontWeight: 700, color: "#93c5fd" }}>+8,1</div>
            <div style={{ fontSize: 11, color: "var(--text-muted)" }}>mdr SEK kvar</div>
        ...
        <div style={{ marginBottom: 12 }}>
          <div style={{ position: "relative", height: 32, background: "var(--bg-inset)", borderRadius: 6, overflow: "hidden" }}>
            <div style={{
        ...
        <div style={{ background: "var(--bg-card)", borderRadius: 6, padding: 10, border: "1px solid var(--border-color)" }}>
          ...
          <table style={{ width: "100%", borderCollapse: "collapse" }}>
            <tbody>
              <tr>
                <td style={{ padding: "3px 0", fontSize: 12, color: "var(--text-secondary)" }}>Q1 2025 (jan–mar)</td>
                <td style={{ padding: "3px 0", fontSize: 12, color: "#f59e0b", textAlign: "right" }}>
                  {mdr(data.q1_2025.revenue_msek)} mdr
                </td>
                <td style={{ padding: "3px 0", fontSize: 10, color: "var(--text-muted)", textAlign: "right" }}>
                  ({data.q1_2025.daily_avg_msek} MSEK/dag)
                </td>
              </tr>
              <tr>
                <td style={{ padding: "3px 0", fontSize: 12, color: "var(--text-secondary)" }}>Svk prognos 2025–2035</td>
                <td style={{ padding: "3px 0", fontSize: 12, color: "#f59e0b", textAlign: "right" }}>
              ...
              <tr>
                <td style={{ padding: "3px 0", fontSize: 12, color: "var(--text-secondary)" }}>Under planperioden 2026–28 (prognos)</td>
                <td style={{ padding: "3px 0", fontSize: 12, color: "#f59e0b", textAlign: "right" }}>
        ...
        <table style={{ width: "100%", borderCollapse: "collapse" }}>
            ...
            <tr>
              {["År", "Intäkter", "Kapacitet", "Tariff", "Kundstöd", "Använt", "Gap"].map((h, i) => (
                <th key={h} style={{
                  ...
                  padding: "6px 6px", fontSize: 11, color: "var(--text-muted)",
          ...
          <tbody>
            {records.filter(r => r.total_used_msek != null).map((r) => {
              ...
              return (
                <tr key={r.year}>
                  <td style={{ padding: "5px 6px", fontSize: 13, color: "var(--text-primary)", borderBottom: "1px solid var(--border-color)" }}>{r.year}</td>
                  <td style={{ padding: "5px 6px", fontSize: 13, color: "#f59e0b", textAlign: "right", borderBottom: "1px solid var(--border-color)" }}>
                    {mdr(r.total_revenue_msek!)}
                  </td>
                  <td style={{ padding: "5px 6px", fontSize: 13, color: "#22c55e", textAlign: "right", borderBottom: "1px solid var(--border-color)" }}>
                    {r.svk_used_for_capacity_msek != null ? mdr(r.svk_used_for_capacity_msek) : "–"}
                  </td>
                  <td style={{ padding: "5px 6px", fontSize: 13, color: "#8b5cf6", textAlign: "right", borderBottom: "1px solid var(--border-color)" }}>
                    {r.svk_tariff_reduction_msek != null ? mdr(r.svk_tariff_reduction_msek) : "–"}
                  </td>
                  <td style={{ padding: "5px 6px", fontSize: 13, color: "#06b6d4", textAlign: "right", borderBottom: "1px solid var(--border-color)" }}>
                    {r.svk_customer_support_msek != null ? mdr(r.svk_customer_support_msek) : "–"}
                  </td>
                  <td style={{ padding: "5px 6px", fontSize: 13, color: "var(--text-primary)", textAlign: "right", borderBottom: "1px solid var(--border-color)", fontWeight: 600 }}>
                    {mdr(r.total_used_msek!)}
                  </td>
                  <td style={{
                    padding: "5px 6px", fontSize: 13, textAlign: "right", borderBottom: "1px solid var(--border-color)",
      ...
      <div style={{ background: "var(--bg-card)", border: "1px solid #22c55e33", borderRadius: 8, padding: 16, marginBottom: 16 }}>
        ...
        <div style={{ display: "flex", gap: 16 }}>
          <div style={{ flex: 1 }}>
            ...
            <div style={{ fontSize: 12, color: "var(--text-muted)" }}>2026–2028 (3 år)</div>
            <div style={{ fontSize: 11, color: "var(--text-muted)" }}>{plan.svk_plan_2026_2028.description}</div>
          </div>
          <div style={{ flex: 1 }}>
            <div style={{ fontSize: 22, fontWeight: 700, color: "#86efac" }}>~19 mdr/år</div>
            <div style={{ fontSize: 12, color: "var(--text-muted)" }}>Genomsnitt per år</div>
            <div style={{ fontSize: 11, color: "var(--text-muted)" }}>Jfr faktiskt inflöde 2024: 21,4 mdr</div>
            <div style={{ fontSize: 11, color: "var(--text-muted)" }}>Jfr Q1 2025-takt: ~43 mdr/år</div>
          </div>
        </div>
      </div>

      {/* ════════ VERIFIED SOURCES ════════ */}
      <div style={{ background: "var(--bg-primary)", borderRadius: 8, padding: 16, border: "1px solid var(--border-color)" }}>
        <div style={{ fontSize: 13, fontWeight: 600, color: "var(--text-primary)", marginBottom: 10 }}>
          🔒 Källor &amp; Metodik
        </div>

        {/* Scope explanation */}
        <div style={{ background: "var(--bg-card)", borderRadius: 6, padding: 10, marginBottom: 12, border: "1px solid var(--border-color)" }}>
          <div style={{ fontSize: 11, fontWeight: 600, color: "#f59e0b", marginBottom: 4 }}>
            ⚠ Två olika mått förekommer i källorna
          </div>
          <div style={{ fontSize: 11, color: "var(--text-secondary)", lineHeight: 1.5 }}>
            <strong style={{ color: "var(--text-primary)" }}>Ei ACER-rapporter</strong> redovisar enbart <em>externa</em> flaskhalsintäkter
            (cross-border: Svk + Baltic Cable). <strong style={{ color: "var(--text-primary)" }}>Svk/SKGS/Second Opinion</strong> anger
          ...
          <div style={{ fontSize: 11, color: "var(--text-muted)", marginTop: 4 }}>
            ...
            5,3 mdr interna snitt). Denna vy visar <strong style={{ color: "var(--text-primary)" }}>totala (Svk inkl interna)</strong> för 2018–2021
            och <strong style={{ color: "var(--text-primary)" }}>Ei samlade (Svk+BC)</strong> för 2022+.
          </div>
        </div>

        {/* Source list */}
        <div style={{ display: "grid", gridTemplateColumns: "auto 1fr auto", gap: "4px 10px", fontSize: 11, color: "var(--text-muted)", lineHeight: 1.6, alignItems: "baseline" }}>
          {/* Primary sources */}
          ...
          <span style={{ color: "var(--text-muted)", fontSize: 10 }}>mars 2025 — intäkter, saldo 65 mdr, användning 2024</span>
          ...
          <span style={{ color: "var(--text-muted)", fontSize: 10 }}>mars 2024 — intäkter 2023, elstöd 26,8 mdr</span>
          ...
          <span style={{ color: "var(--text-muted)", fontSize: 10 }}>mars 2021 — Svk 2 871 MSEK externt, BC 364 MSEK</span>
          ...
          <span style={{ color: "var(--text-muted)", fontSize: 10 }}>juni 2023 — 6,1 mdr godkänd</span>
          ...
          <span style={{ color: "var(--text-muted)", fontSize: 10 }}>jun 2021 — totalt 8,1 mdr 2020 (inkl interna snitt)</span>
          ...
          <span style={{ color: "var(--text-muted)", fontSize: 10 }}>maj 2025 — Q1 2025 10,5 mdr, prognos 2025–2035</span>
          ...
          <span style={{ color: "var(--text-muted)", fontSize: 10 }}>dec 2023 — 2018: 1,6 mdr, 2019: 2,2 mdr (citat Svk CFO)</span>
          ...
          <span style={{ color: "var(--text-muted)", fontSize: 10 }}>nov 2025 — saldo 80 mdr, Riksgäldskonto</span>
        </div>

        <div style={{ fontSize: 10, color: "var(--text-muted)", marginTop: 12, paddingTop: 8, borderTop: "1px solid var(--border-color)" }}>
